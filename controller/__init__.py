...
class IRLException(Exception):
    pass


class IRLProtocol(serial.threaded.LineReader):

    TERMINATOR = b'\r\n'

    def __init__(self):
        super(IRLProtocol, self).__init__()
        self.alive = True
        self.responses = queue.Queue()
        self.events = queue.Queue()
        self._event_thread = threading.Thread(target=self._run_event)
        self._event_thread.daemon = True
        self._event_thread.name = 'irl-event'
    ...
    def command(self, command, response='OK', timeout=5):
        """
        Set a command and wait for the response.
        """
        with self.lock:  # ensure that just one thread is sending commands at once
            ...
            while True:
                try:
                    ...
                    # logging.info("%s -> %r" % (command, line))
                    if line == response:
                        return lines
                    else:
                        lines.append(line)
                except queue.Empty:
                    raise IRLException('Command timeout ({!r})'.format(command))
